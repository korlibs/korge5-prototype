...
class VfsFileTest {
    ...
    fun testLocalRead() = suspendTest({ Platform.isJvm }) {
        ...
        //println("************************************")
        localCurrentDirVfs.parent.list().filter { it.baseName == "settings.gradle.kts" }.collect {
            if (it.isFile()) {
                //println("$it: ${it.readAll().size}")
                processedFileNamesWithSize += "${it.baseName}:${it.readAll().isNotEmpty()}"
            }
        }
        //println("************************************")
        assertEquals(listOf("settings.gradle.kts:true"), processedFileNamesWithSize)
    }

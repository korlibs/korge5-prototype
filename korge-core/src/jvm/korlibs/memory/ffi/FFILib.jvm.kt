        ...
        lib.functions.associate { nfunc ->
            ...
            nfunc.name to Proxy.newProxyInstance(
                ...
                object : InvocationHandler {
                    val func: Function? = if (lib.kind != FFILibKind.WASM) nlib?.getFunction(nfunc.name) ?: error("Can't find function ${nfunc.name}") else null
                    override fun invoke(proxy: Any?, method: Method, args: Array<out Any>?): Any? {
                        if (lib.kind == FFILibKind.WASM) {

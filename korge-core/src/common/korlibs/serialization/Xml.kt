...
data class Xml(
    ...
    companion object {
        ...
        fun Tag(tagName: String, attributes: Map<String, Any?>, children: List<Xml>): Xml =
            Xml(
                Xml.Type.NODE,
                tagName,
                attributes.filter { it.value != null }.map { it.key to it.value.toString() }.toMap(),
                children,
                ""
            )
                ...
                fun level(): Level {
                    ...
                    while (stream.hasNext()) {
                        ...
                        when (tag) {
                            ...
                            is Xml.Stream.Element.Text -> children.add(
                                (if (tag.cdata) Xml.CData(tag.text) else Xml.Text(
                                    tag.text
                                ))
                            )

                            is Xml.Stream.Element.OpenCloseTag -> children.add(
                                Xml.Tag(
                                    tag.name,
                                    tag.attributes,
                                    listOf()
    ...
    object Encode {
        ...
        fun quoteParams(map: Map<String, Any?>): String =
            map.entries.joinToString(" ") { it.key + "=" + quote(it.value) }
    ...
    object Entities {
        ...
        private val charToEntity =
            linkedMapOf('"' to "&quot;", '\'' to "&apos;", '<' to "&lt;", '>' to "&gt;", '&' to "&amp;")
            ...
            while (!r.eof) {
                ...
                val res: Element = when {
                    ...
                    r.tryExpect("!--") -> {
                        ...
                        while (r.peekChar() != '?' && r.peekChar() != '/' && r.peekChar() != '>') {
                            ...
                            if (r.tryExpect("=")) {
                                ...
                                attributes[key] = when {
                                    ...
                                    argsQuote != null -> Xml.Entities.decode(
                                        argsQuote.substring(
                                            1,
                                            argsQuote.length - 1
                                        )
                        ...
                        r.skipExpect('>')
                        when {
                            processingInstruction || processingEntityOrDocType -> Element.ProcessingInstructionTag(
                                name,
                                attributes
                            )
